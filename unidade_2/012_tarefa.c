...
int main() {
    ...
    double t_dynamic = run_simulation("dynamic");
    double t_guided  = run_simulation("guided");
    ...
    printf("Speedup Dynamic: %.2f\tEficiência: %.2f%%\n", t_serial / t_dynamic, 100.0 * (t_serial / t_dynamic) / n_threads);
    printf("Speedup Guided : %.2f\tEficiência: %.2f%%\n", t_serial / t_guided, 100.0 * (t_serial / t_guided) / n_threads);
...
gcc -fopenmp tarefaNPAD_12.c -o tarefaNPAD_12 -lm
